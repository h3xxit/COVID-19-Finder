...
public class PasswordAndCode extends AppCompatDialogFragment {
    ...
    public PasswordAndCode(FragmentManager fragmentManager, UserAPI userAPI, Context context, Activity activity, TextView emailTextView, Button infectedBtn, String emailOrPhone) {
            ...
            builder.setView(view)
                    ...
                    .setNegativeButton(R.string.back, new DialogInterface.OnClickListener() {
                        ...
                        public void onClick(DialogInterface dialogInterface, int i) {
                            RegisterEmailOrPhone registerEmailOrPhone = new RegisterEmailOrPhone(frgManager, userAPI, LoginRegisterContext, LoginRegisterActivity, emailTextView, infectedBtn);
    ...
    public void onResume() {
        ...
        Button resendButton = globalDialog.getButton(AlertDialog.BUTTON_NEUTRAL);
    ...
    private void performResendButton() {
        ...
        if(emailOrPhone.contains("@"))
            sendConfirmationEmail(emailOrPhone);
        else
            sendConfirmationSms(emailOrPhone);
    }
    ...
    private void performOkButtonAction() {
        ...
        if(checkPassword(password, passwordConfirmation)){
            if(PasswordStrength.WEAK != PasswordStrength.calculate(password)) {
                register(code, password, emailOrPhone);
            }
            else {
    ...
    private void register(Integer code, String password, String email) {
        if (getContext() != null && getActivity() != null) {
            if (ContextCompat.checkSelfPermission(LoginRegisterContext,
                ...
                RegisterWithCodeDto registerWithCodeDto = new RegisterWithCodeDto(email, password, code);
                userAPI.createUser(registerWithCodeDto).enqueue(new Callback<ResponseBody>() {
                    @Override
    ...
    private void sendConfirmationEmail(String email){
        if(getContext()!=null && getActivity()!=null)
        {
            if (ContextCompat.checkSelfPermission(LoginRegisterContext,
                ...
                userAPI.sendConfirmationEmail(emailOrSmsDto).enqueue(new Callback<ResponseBody>() {
                    @Override
                    public void onResponse(Call<ResponseBody> call, Response<ResponseBody> response) {
                        if(response.isSuccessful()){
                            progressBar.setVisibility(View.INVISIBLE);
                            ...
                            if(response.code()==500) {
                                progressBar.setVisibility(View.INVISIBLE);
                                Toast.makeText(LoginRegisterContext, R.string.email_error, Toast.LENGTH_LONG).show();
                            } else {
                                progressBar.setVisibility(View.INVISIBLE);
    ...
    private void sendConfirmationSms(String phone){
        if(getContext()!=null && getActivity()!=null)
        {
            if (ContextCompat.checkSelfPermission(LoginRegisterContext,
                    Manifest.permission.INTERNET)
                ...
                userAPI.sendConfirmationSms(emailOrSmsDto).enqueue(new Callback<ResponseBody>() {
                    @Override
                    public void onResponse(Call<ResponseBody> call, Response<ResponseBody> response) {
                        if(response.isSuccessful()){
                            progressBar.setVisibility(View.INVISIBLE);
